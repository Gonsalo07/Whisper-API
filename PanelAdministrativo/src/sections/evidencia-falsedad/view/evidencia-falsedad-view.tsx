...
import {
  ...
  deleteEvidencia,
  fetchEvidenciaById,
  type EvidenciaFalsedad,
} from 'src/services/evidencia-falsedad-api';

import { Iconify } from 'src/components/iconify';
import { Scrollbar } from 'src/components/scrollbar';

import { TableNoData } from '../../user/table-no-data';
import { useTable } from '../use-evidencia-falsedad-table';
import { UserTableHead } from '../../user/user-table-head';
import { emptyRows, getComparator } from '../../user/utils';
import { TableEmptyRows } from '../../user/table-empty-rows';
import { UserTableToolbar } from '../../user/user-table-toolbar';
import { EvidenciaFalsedadDialog } from '../evidencia-falsedad-dialog';
import { EvidenciaFalsedadTableRow, type EvidenciaFalsedadRow } from '../evidencia-falsedad-table-row';

export function EvidenciaFalsedadView() {
  const table = useTable();

  const [rows, setRows] = useState<EvidenciaFalsedadRow[]>([]);
  const [loading, setLoading] = useState(true);
  const [filterName, setFilterName] = useState('');

  const [dialogOpen, setDialogOpen] = useState(false);
  const [dialogMode, setDialogMode] = useState<'create' | 'edit'>('create');
  const [selected, setSelected] = useState<EvidenciaFalsedad | null>(null);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const mapped: EvidenciaFalsedadRow[] = data.map((e) => ({
        id: e.id!.toString(),
